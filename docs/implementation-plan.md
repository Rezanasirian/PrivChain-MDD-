...
- [x] **Capability-conditioned logit MoE** (ADR-0028). The ADR-0027 verdict says drop audio and video; the thesis says the contribution is *missing-modality* robustness, and a text-only model has nothing to offer an audio-only client. So each modality now emits its own logit and a masked softmax over the **present** modalities mixes them — an absent modality weighs exactly zero and the rest renormalize. Training shows each participant one capability per epoch, cycling the committed deployment mix (4 full : 3 audio+text : 2 audio-only : 1 text-only) so DP sensitivity stays at the participant. One loss, no new coefficients; the gate prior is recorded, not swept. **One pre-registered test**: Δ = AUC_audio_only(MoE) − AUC_audio_only(baseline), paired participant bootstrap; `min` over capabilities is descriptive only. `src/privchain/fusion/moe_model.py`, `src/privchain/training/capability_schedule.py`, `src/privchain/federated/capability_patterns.py`, `scripts/run_capability_moe.py`. **Run on the real corpus at 20 seeds** (`experiments/phase1/phase1_capability_moe_20260902_201447`): the pre-registered test returns a **null** — Δ_audio_only = +0.012, CI [-0.029, +0.052]. Logit fusion moves the worst committed capability in the right direction but not detectably, and it costs elsewhere (text_only -0.060, full -0.029; macro 0.642 against 0.665). Not adopted. Two observations worth carrying forward, neither tested: the capability schedule itself lifted the *baseline* architecture's full-modality AUC from 0.630 (ADR-0027 ladder, same pool and folds, no schedule) to 0.703, so training under the deployment mix looks more valuable than the fusion change; and the MoE gate settled at audio 0.40 / text 0.40 rather than holding its text prior, which is not what the modality strengths predict.
