...
| H5 | **Empirical evaluation** of the prototype against prior work in terms of diagnostic accuracy, parameter-group noise allocation under a composed participant guarantee, robustness to asymmetric access, and inference latency under matched compute budgets | Chapter 4 tables + Chapter 5 analysis |
...
- [x] **Segment-aligned, quality-gated architecture** (ADR-0027). The session-level view collapsed a 15-minute interview to one vector per modality, and the measured consequence was that the tri-modal model was not significantly better than text alone. `daic_woz.segments.enabled` now cuts all three modalities at the same turn-group boundaries — audio restricted to the participant's own speech intervals, video keeping the full envelope — and `SegmentGatedNetwork` fuses them **per segment** under a masked softmax that reads each modality's measured quality, then pools the segments by additive attention. Adds per-sample modality dropout, an optional Huber PHQ-8 loss, and average precision (`pr_auc`) to the metrics. `src/privchain/data/segment_alignment.py`, `src/privchain/fusion/{segment_model,quality_gated_fusion,factory}.py`, `scripts/run_segment_architecture.py`. **Run on the real corpus at 20 seeds** (`experiments/phase1/phase1_segment_architecture_20260902_185430`) and the verdict does not favour the architecture: `segments+attn` is the only arm that moves (+0.107 ROC-AUC, 77/100 folds, CI [-0.034, +0.255], not significant), while `segments+av+gated` is significantly **worse** than the baseline (-0.111, CI [-0.222, -0.006]). Session-level audio and video alongside segmented text actively hurt. The full segment-aligned network has no statistical case for replacing the committed model.
- [x] **Capability-conditioned logit MoE** (ADR-0028). The ADR-0027 verdict says drop audio and video; the thesis says the contribution is *missing-modality* robustness, and a text-only model has nothing to offer an audio-only client. So each modality now emits its own logit and a masked softmax over the **present** modalities mixes them — an absent modality weighs exactly zero and the rest renormalize. Training shows each participant one capability per epoch, cycling the committed deployment mix (4 full : 3 audio+text : 2 audio-only : 1 text-only) so DP sensitivity stays at the participant. One loss, no new coefficients; the gate prior is recorded, not swept. **One pre-registered test**: Δ = AUC_audio_only(MoE) − AUC_audio_only(baseline), paired participant bootstrap; `min` over capabilities is descriptive only. `src/privchain/fusion/moe_model.py`, `src/privchain/training/capability_schedule.py`, `src/privchain/federated/capability_patterns.py`, `scripts/run_capability_moe.py`. **Run on the real corpus at 20 seeds** (`experiments/phase1/phase1_capability_moe_20260902_211640`): the pre-registered test returns a **null** — Δ_audio_only = +0.028, CI [-0.005, +0.062]. Logit fusion moves the worst committed capability in the intended direction and the interval now only just includes zero, but it costs heavily elsewhere: text_only 0.564 against the baseline's 0.731, macro 0.635 against 0.686. Not adopted. Per ADR-0028's scope note the verdict is on *this* MoE, not on late-logit fusion in general — the arms also differ in head depth, and the text_only drop cannot come from the gate, which weighs text exactly 1 when text is all there is. Two earlier 20-seed runs are superseded and kept for the record: 3cf89f3 inherited `phq_loss_weight: 0.1` and rotated its selection split; a7741a0 fixed those but selected checkpoints under a batch-count-averaged loss that gave `text_only` 47% of the criterion instead of 25%. The estimate has been stable across all three (+0.012 → +0.023 → +0.028), so the protocol defects moved the number without changing the verdict.
...
- **Revised 2026-08-13 (ADR-0021).** Run on **real DAIC-WOZ** under the shared protocol for the first time, via the new `scripts/run_federated_comparison.py`. The old `run_capability_federated.py` selected the best global model on the same loader it reported, ran one seed and quoted no interval; it is kept for the mock path but must not be cited. Added label-skewed (Dirichlet) partitioning next to IID — a protocol whose pitch is heterogeneity had only ever been tested on the homogeneous case — and early stopping on the selection split, since federated arms previously ran a fixed round budget while the centralized baseline stopped at its best epoch. `num_rounds` 20 → 120 with patience 40.
- **Flower actually executed (ADR-0021).** Running it against flwr 1.33 exposed two runtime defects in the never-executed adapter: clients were constructed in the parent process, so Ray had to pickle CUDA tensors into every worker, and workers were granted no GPU. Both failed **silently** — Flower logs "received 0 results and N failures" and continues with the untrained initial parameters, which reads as a converged run. Clients are now built inside the worker and Ray is given a GPU slice per client.
...
- **Revised 2026-08-13 (ADR-0018) — H1's central claim is now tested, and the way it used to be tested was unsound.** The adaptive-vs-uniform comparison matched its arms on `sum(eps)/3`, the arithmetic mean of the per-modality ε. That is not equal privacy: a participant spends the RDP *composition* over the three encoders plus the shared head (ADR-0009), which is dominated by the loosest mechanism, so the deliberately uneven adaptive arm was quietly getting **more** real privacy than the uniform one — a bias pointing straight at the hypothesis under test. Arms are now scaled to a matched **composed participant ε** via `scale_to_participant_epsilon`, and `scripts/run_allocation_comparison.py` refuses to report if the match drifts past 1%. A third `anti_adaptive` control arm (ε ∝ risk) makes the result readable rather than leaving a two-arm gap that could be seed noise. The DP-SGD loop, previously copied into three scripts, now lives once in `src/privchain/privacy/dp_training.py`; the ε sweep was re-run afterwards against its committed numbers to prove the refactor changed nothing.
  - **Measured (participant ε = 8, 86 train / 34 dev, 3 seeds).** Budgets matched to 0.04%. The adaptive arm assigns the audio parameter group ε 1.96 and video ε 2.02 against uniform's 3.62, so it injects more noise into the groups associated with the modalities ADR-0017 ranked as riskiest. **Amendment 2026-09-12 (ADR-0009): these are parameter-group calibration targets/mechanism costs, not end-to-end guarantees for changing an audio or video record.** Because the fused loss lets any modality record affect every encoder gradient, the proved guarantee for the complete released model is the matched participant ε = 8 in both arms; the previous “~1.8× tighter modality protection” wording is withdrawn. **The utility half also does not hold**: uniform 0.549 ± 0.067, adaptive 0.515 ± 0.090, anti_adaptive 0.536 ± 0.076 — a 0.034 spread against a 0.090 std, i.e. no measured difference, with the deliberately-wrong control scoring like the rest. The cause is upstream: at these budgets every arm is already near chance (cf. non-private 0.740, clipping-only 0.596), so there is no surviving utility left to reallocate. Chapter 1 must describe H1 as auditable risk-guided **noise allocation under a common participant-level bound**, with no demonstrated utility gain or modality-record-specific end-to-end bound.
...
- ⚠️ **Measured on real data 2026-08-13 (ADR-0021) — the Definition of Done is NOT met on the accuracy criterion.** Five arms × {IID, Dirichlet, 5-client} × 3 seeds, paired bootstrap against FedAvg. **Nothing separates, in any configuration.** `capability − fedavg` is +0.024 (IID, p=0.567) and +0.036 (skewed, p=0.208) — the direction H2 predicts, slightly larger when clients are heterogeneous — but it **reverses to −0.043 with 5 clients**, so a mechanism whose sign depends on client count has not been demonstrated. Reputation (+0.016) and distillation (+0.008…+0.020) add nothing anywhere. For scale: federation itself costs ~0.17 ROC-AUC (centralized 0.740 vs federated ~0.56) and *even that* is not statistically resolvable (p = 0.12–0.28) on 34 dev sessions. The per-pattern table is more revealing than the headline: a federated model given **only text** scores 0.685–0.689 against 0.555 given everything, so the audio and video encoders are contributing noise the fusion layer must overcome — ADR-0016's finding arriving from a different direction. Chapter 4 must report a bound, not an improvement.
...
- [x] Write chaincode in Go with 4 functions + read helpers, input validation, explicit errors, and `shimtest` (MockStub) unit tests. `chaincode/privchain-cc/` (ADR-0006):
  ...
  - `LogPrivacyBudget(clientID, group, round, epsilonIncremental, epsilonCumulative)` — **append-only** (consumed ε never overwritten, CLAUDE.md §7)
...
- ✅ **Met for real on 2026-08-13 (ADR-0022).** A genuine Fabric network now runs **without Docker**, which is impossible on the deployment container (no `CAP_SYS_ADMIN`; the seccomp profile blocks `unshare`, so rootless engines are out too). Fabric needs Docker only to build/run the chaincode container, so peer and orderer run as native binaries and the chaincode runs as a process via **Chaincode-as-a-Service** — the arrangement Kubernetes deployments use. `scripts/fabric/{setup_network,deploy_chaincode,verify_ledger,teardown}.sh` plus a Go REST gateway bring the whole stack up from nothing. Verified: writes read back, **survive a peer restart**, and the *real* chaincode rejects a duplicate privacy-budget entry and a republished subgraph. `run_federated_with_ledger.py --backend fabric_rest` completes 3 rounds and reads its audit trail back from the ledger, with per-modality consumed ε converging exactly on the configured caps (audio→2.0, video→4.0, text→8.0); final channel height 68. **Standing it up exposed nine defects in never-executed code** — chief among them that a `.gitignore` rule matching the *directory* `chaincode/privchain-cc/` had silently kept `identity.go` and `go.sum` out of git, so the chaincode had never compiled and its 12 shimtest tests had never run (they pass now), and that the raft WAL defaulted outside the purge path, making every rerun die with what looked like ledger corruption.
- **Definition of Done:** One full round of federated training runs with real reads/writes against the blockchain (not an in-memory simulation). ✅ **Met against live Fabric on 2026-08-13** using the Chaincode-as-a-Service deployment documented in ADR-0022; Docker is not a requirement of this deployment target.
...
- [x] **Run on real DAIC-WOZ 2026-08-14 (ADR-0023).** The harness had no `--daic-config` and built `MockDaicWozDataset` directly, so the one artifact Chapter 4 is assembled from was the only one never run on the corpus. Folds now come from the pooled official train+dev (141 sessions) with `full_test_split` reserved for one read per method; every arm follows the ADR-0015 protocol. Wiring it exposed that input dims came from the mock config (40/49/64 rather than 74/20/768), that the DP and federated arms were pinned to CPU, that stratification decoded all 141 sessions to read 141 integers, and — after the first run put every federated arm at F1 exactly 0.000 — that a mock-era `rounds: 5` was overriding `federated.yaml`'s 120, training the proposed method for 5 rounds against a centralized arm allowed 200 epochs. Fixing that surfaced two more: the simulations' `device` and `early_stopping_patience` were never passed, and the fold's **scored** split was being used as the per-round validation loader, which with early stopping would have selected the global model on the number being reported.
- **Measured, including what does not support the thesis.** Centralized reproduces the Phase 1 baseline (CV ROC-AUC 0.680 ± 0.149; official test **0.709 ± 0.052**), which is the check that the protocol is real. But **H2 is not demonstrated**: all four federated arms sit at CV ROC-AUC 0.573–0.584 against a ±0.25 fold spread, and removing reputation moves the mean 0.006 — matching Phase 4's paired-test conclusion (ADR-0021). **H1's utility claim is not demonstrated either**: the adaptive and uniform DP arms return *identical* F1/ROC-AUC/accuracy (only `loss_mean` differs), and Phase 3's re-run supplies the control that settles it — anti_adaptive − uniform = 0.000, p = 1.0, i.e. giving the riskiest modalities the *most* budget scores the same as uniform, so the allocation axis is inert at ε = 8 on 141 sessions. H1's *privacy* claim stands on Phase 6 independently. Open and unexplained: every federated arm falls to or below chance on the official test split (0.422–0.467, seed spread ±0.01) while centralized holds 0.709.
- **Re-measured 2026-09-13 under the re-locked protocol (`PRE-REGISTRATION-2026-09-13.md`, ADR-0033).** The campaign was rerun after the preprocessing investigation closed (ADR-0030/0031/0032, all null) and the federated local optimizer was decided on train-only inner CV (ADR-0033, also null — the defaults stand). Predictions are now persisted for the official read, so the arms can finally be compared **paired at participant level** (47 test participants, 14 positive, 3 seeds averaged per participant): `experiments/phase7/phase7_final_evaluation_20260913_104945/official_test_paired.md`.
  **This changes the Phase-4/Phase-7 conclusion.** Official test ROC-AUC: centralized 0.705, **proposed 0.584**, proposed−reputation 0.573, personalized 0.483, FedAvg 0.452. Paired: `proposed − fedavg` = **+0.193, 95% CI [+0.103, +0.303], p < 0.001**, and `proposed − personalized` = +0.141 [+0.071, +0.226] — so H2's protocol **is** demonstrated on the held-out split, unlike in the 2026-08-13 dev-split test. `proposed − proposed_no_reputation` = +0.017 [+0.002, +0.038], p = 0.029: reputation contributes a small but resolvable amount. And `centralized − proposed` = +0.106 [-0.035, +0.248], p = 0.150 — **the proposed framework is not statistically separable from centralized**, while plain FedAvg (+0.299) and personalized (+0.247) both are.
  **Reported against it:** the 10-fold CV on the pooled train+dev split ranks the arms differently (FedAvg 0.502 against proposed 0.469). With 113 pooled and 47 test participants both estimates are noisy; the disagreement is disclosed in the lock and must be stated in Chapter 4 rather than resolved by choosing the flattering split. H1's utility claim remains unsupported: adaptive and uniform DP allocation both give 0.409.
